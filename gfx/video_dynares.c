...
void dynares_video_show_info(int base_width, int base_height, int interlaced, float hz)
{
    char msg[128];
    settings_t *settings = config_get_ptr();

    char *mode = "p"; // Always use 'p'

    // Debugging logs
    RARCH_LOG("[DynaRes]: Showing video info. Base resolution: %dx%d, Interlaced: %d, Hz: %.3f\n", base_width, base_height, interlaced, hz);

    // Check if the height is over 300 and halve the resolution if so
    if (base_height > 300)
    {
        RARCH_LOG("[DynaRes]: Height is over 300. Halving resolution.\n");
        base_width /= 2; // Halve the base width
        base_height /= 2; // Halve the base height
    }

    snprintf(msg, sizeof(msg),
             "%ux%u%s %.3fHz.", base_width, base_height, mode, hz);
...
void adjust_interlaced_height(unsigned *height, timing *t)
{
    if (t->interlaced)
    {
        RARCH_LOG("[DynaRes]: Adjusting interlaced height. Original height: %u\n", *height);
        t->interlaced = 0; // Set to non-interlaced after halving
        *height /= 2;
        t->vvv = *height;
        t->v_total /= 2;
        t->fps *= 2; // Ensure the correct frame rate is set
        RARCH_LOG("[DynaRes]: Adjusted height: %u, Frame rate: %.3f\n", *height, t->fps);
    }
}
